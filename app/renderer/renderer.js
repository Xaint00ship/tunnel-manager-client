...
const guardUnavailable =
  'Строгая firewall-защита пока доступна только в Windows-ветке. На macOS используется TUN sing-box без persistent kill-switch.'
...
function renderStatus() {
  ...
  if (st.killSwitchSupported === false) $('killSwitch').disabled = true
  ...
  const protection =
    st.killSwitchSupported === false
      ? 'Строгая защита на macOS пока отключена'
      : st.trafficProtected
...
function renderRules() {
  ...
  if (st.killSwitchSupported === false) {
    $('killSwitch').checked = false
    $('guardNote').hidden = false
    text('guardNote', guardUnavailable)
  } else {
    $('guardNote').hidden = !cfg.killSwitch
    text(
      'guardNote',
      'Прямые исключения отключаются. Закрытие приложения не снимает блокировку; для восстановления интернета нажмите «Отключить» на экране подключения.'
    )
  }
  const box = $('siteList')
  box.replaceChildren()
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  $('killSwitch').onchange = () => {
    if (st.killSwitchSupported === false) {
      $('killSwitch').checked = false
      text('advancedMessage', guardUnavailable)
      return
    }
