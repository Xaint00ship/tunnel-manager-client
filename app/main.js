...
const APP_TITLE = 'Tunnel Manager Client'
...
const SB_DIR = path.join(RES, 'tools', 'sing-box'),
  SB_EXE = path.join(SB_DIR, IS_WINDOWS ? 'sing-box.exe' : 'sing-box')
const GUARD = IS_WINDOWS ? path.join(RES, 'scripts', 'kill-switch.ps1') : null
const CURL_EXE = IS_WINDOWS ? 'curl.exe' : 'curl'
...
const state = {
  platform: PLATFORM,
  killSwitchSupported: IS_WINDOWS,
  clientRunning: false,
  route: null,
  egress: null,
  proxyEgress: null,
...
function loadConfig() {
  ...
  return normalizeConfig(validateConfig(JSON.parse(fs.readFileSync(CONFIG, 'utf8').replace(/^﻿/, ''))))
}
function saveConfig(cfg) {
  const clean = normalizeConfig(validateConfig(cfg))
  atomicJson(CONFIG, clean)
  return clean
}
function normalizeConfig(cfg) {
  return IS_WINDOWS ? cfg : { ...cfg, killSwitch: false }
}
function readDp() {
  try {
...
async function firewall(off = false, status = false) {
  if (!IS_WINDOWS) {
    ...
    if (!off && !status)
      throw new Error('Persistent firewall guard is only available in the Windows build')
    return false
  }
...
async function checkOtherTunnel() {
  if (IS_MAC) {
    try {
      const output = await command('netstat', ['-rn', '-f', 'inet'], {}, 8000)
      const line = output
        .split(/\r?\n/)
        .find((value) => /^default\s+/.test(value.trim()))
      const iface = line?.trim().split(/\s+/).at(-1)
      return /^(utun|ipsec|ppp)\d*$/i.test(iface || '') ? iface : null
    } catch {
      return null
    }
  }
...
async function applyVpn() {
  ...
  pushStatus()
  const tunnelAlias = IS_MAC ? '' : 'magnetgate-' + crypto.randomBytes(6).toString('hex')
...
if (!app.requestSingleInstanceLock()) app.exit(0)
  ...
  app.whenReady().then(async () => {
    createWindow()
    pushLog(APP_TITLE + ' ' + app.getVersion() + ' started')
    sweepLeftovers()
    try {
      await firewall(false, true)
      if (guardReady)
