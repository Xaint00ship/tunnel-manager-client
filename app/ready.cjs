...
// The SOCKS inbound follows TUN initialization. Both it and our authenticated control endpoint must
// be ready; a child PID by itself does not mean the OS finished creating the TUN device.
async function waitForEngineReady(child, config, signal, timeoutMs = 30000) {
  ...
  try {
    ...
    if (timeout.aborted && !signal.aborted)
      throw new Error(`TUN did not become ready within ${timeoutMs / 1000} seconds`)
    throw err
  }
}
