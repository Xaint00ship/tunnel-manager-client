...
- стек утвержден;
- D2 подтвержден на walking skeleton или пересмотрен с обновлением ТЗ;
- тестовый `client-api` на тестовом сервере регистрирует и снимает peer на `wg0` и отдает список, забранный у дашборда;
...
- оба транспорта проходят leak-тесты и M16-M17; пробный период и админ-панель проверены по M18-M19.
