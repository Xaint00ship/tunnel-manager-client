# Tunnel Manager Client: дорожная карта

Принцип: сначала снимаем главный риск, потом строим продукт. Главный риск - native-часть (туннель, маршруты, fail-closed, helper) на двух ОС. Поэтому после фазы решений идет walking skeleton, а не оболочка приложения. TypeScript-ядро делается параллельно: оно ничем не рискует и не должно ждать.

## Phase 0: решения, спецификация, окружение

Цель: согласовать направление и убрать неизвестные, от которых зависит весь native-код.

Результаты:

- public repository, `README.md`, техническое задание, roadmap;
- явное разделение приватного серверного контура и коммерческого клиентского приложения;
- decision records ADR-001 - ADR-005: транспорт, путь на macOS, список и DNS, размещение control plane, второй транспорт;
- репозиторий `tunnel-client-api` создан, CI и деплой на зарубежный сервер настроены;
- D1 подтвержден по коду серверного репозитория: транспорт WireGuard; D3 исправлен: список - курируемые IP из дашборда, резолва доменов нет;
- D4 подтвержден владельцем: свой зарубежный сервер; на нем разворачиваются `wg0` и `client-api` (ТЗ 15.5, 15.6, ADR-004) - до фазы 1;
- тестовое окружение по `NFR-047`: тестовый сервер с `wg0` и `client-api`, тестовый бот, тестовые коды, стенд leak-тестов, физическая Windows-машина;
- минимальный тестовый `client-api` для фазы 1, отдельный от production backend фазы 6: регистрация и снятие peer WireGuard и EAP-учетки IKEv2 по фиксированным тестовым устройствам (strongSwan с тестовым сертификатом на том же сервере), `GET /client/v1/routes` из списка дашборда или статического файла, `GET /client/v1/endpoint` с явным `probeAddress` и достижимой серверной целью TCP 53 для WireGuard; без активации через бота, подписок, отчетов и admin API;
- запрошены сертификаты: Apple Developer ID и code signing для Windows;
- стартовый список issues.

Критерии завершения:

- стек утвержден;
- минимальный тестовый `client-api` на тестовом сервере регистрирует и снимает peer на `wg0` и EAP-учетку, отдает список, забранный у дашборда, и endpoint для тестовых устройств; D2 в этой фазе не проверяется - это результат фазы 1;
- платформы MVP подтверждены: macOS Apple Silicon и Windows 10/11 x64, обе выходят одним релизом;
- тестовый сервер с `wg0` и `client-api` и тестовый бот доступны разработчикам.

## Phase 1: walking skeleton

Цель: доказать на обеих платформах, что native-часть реализуема, до написания UI и backend.

Результаты на каждой платформе, без UI и без production backend (используется минимальный тестовый `client-api` из фазы 0):

- privileged helper на macOS и служба на Windows с IPC и проверкой вызывающего;
- поднятие WireGuard-туннеля до тестовой точки из helper;
- второй драйвер IKEv2: RAS на Windows, Personal VPN на macOS; снятие catch-all и маршруты на его интерфейсе;
- добавление и удаление одного маршрута через туннель;
- блокировка одного CIDR на физических интерфейсах: pf на macOS, WFP на Windows;
- переживание `kill -9` приложения: блокировка остается, helper снимает ее по таймауту;
- `applyConfiguration` с журналом: убийство helper после каждого шага и восстановление при старте без утечки (ТЗ 13.6, `T-002`);
- helper сам следит за drift и шлет события, переживает закрытие приложения (`FR-050`, `FR-186`);
- смена DNS при поднятии и потере туннеля (`FR-167`, `FR-167b`);
- минимальный leak-тест L1, L2, L5 с захватом трафика.

Параллельно и независимо от skeleton - Core TypeScript:

- `route-engine`, `api-client`, `local-store`, `shared-types`;
- state machine для connect/sync/error;
- модель redacted error report;
- mock backend;
- unit tests на route logic, cache, backoff, redaction.

Критерии завершения:

- skeleton проходит L1, L2, L5 на macOS и на Windows, на обоих транспортах;
- D2 подтвержден: ADR-002 подтвержден на skeleton, либо заменен на NetworkExtension с пересчетом сроков фазы 3 и обновлением ТЗ;
- core-пакеты покрыты тестами и не содержат ОС-специфичного кода;
- принято решение "идем дальше" или "меняем план".

Ориентир по сроку: 4-6 недель с учетом второго транспорта, WireGuard первым, IKEv2 вторым. Если skeleton не проходит за 8 недель, план пересматривается до начала фазы 2, а не после.

## Phase 2: оболочка приложения и интеграция core

Цель: собрать приложение вокруг уже проверенного ядра.

Результаты:

- Tauri v2 app, React + TypeScript frontend;
- package manager и workspace structure, linting/formatting;
- CI с двумя раннерами: macOS и Windows;
- окно приложения с простым пользовательским status screen;
- экран активации: вход через бота, пробный период, ручной код;
- выключенный по умолчанию placeholder для "Режима эксперта" и каркас раздела «Администрирование» за ролью;
- core подключен к UI через status model;
- fake `NativeAdapter` для разработки UI без прав и без туннеля;
- команды для локальной разработки на обеих ОС.

Критерии завершения:

- приложение запускается локально на macOS и на Windows;
- CI проходит TypeScript checks, unit tests и basic build на обоих раннерах;
- route list из mock API получается и кешируется;
- обычный UI не показывает техническую диагностику без режима эксперта.

## Phase 2 → 3: тестовая авторизация и контракт backend

Зависимость до интеграции Phase 3: расширить минимальный `client-api` Phase 0 сценариями `device-code`/`approve`/`device-token`, `activate`, `refresh`, `DELETE device`, `profile`, `endpoint` (включая `probeAddress` и `ikev2-ack`). Подключить тестового бота для deep link и ручного кода, тестовые активные/истекшие/отозванные аккаунты и роли. Схемы, fixture-векторы и реализация auth-контрактов хранятся в `tunnel-client-api` и затем используются Phase 6; временный несовместимый протокол не создается.

Критерий готовности к интеграции: оба способа входа, потеря ответа refresh, сохранение секретов, профиль и отзыв проходят контрактные тесты на стенде, секреты не попадают в ответы без аутентификации или в логи. Production-интеграция с RU, полный admin API, trial и эксплуатационная готовность остаются в Phase 6. Этот этап можно вести во время Phase 2; Phase 1 не ждет авторизацию и работает с фиксированными устройствами.

## Phase 3: Tunnel Control MVP на macOS и Windows

Цель: сделать приложение реально полезным на обеих desktop-платформах.

Обе платформы делаются параллельно против одного контракта `NativeAdapter` из раздела 13.2 ТЗ. Skeleton из фазы 1 оборачивается в адаптеры, а не переписывается.

Общие результаты:

- зафиксированный контракт native adapter;
- connect/disconnect из UI по обоим транспортам, ручной выбор транспорта в режиме эксперта;
- status detection и определение active tunnel interface;
- basic route apply/remove;
- permission flow там, где нужны повышенные права;
- активация устройства против расширенного тестового `client-api` после этапа Phase 2 → 3 обоими способами: deep link с подтверждением в тестовом боте и ручной код; генерация ключевой пары на клиенте;
- общий набор интеграционных тестов, который гоняется на обеих ОС.

macOS:

- native adapter поверх helper из фазы 1;
- secure storage через Keychain.

Windows:

- native adapter поверх службы из фазы 1;
- named pipe с ACL и проверкой вызывающего;
- secure storage через Credential Manager или DPAPI;
- сертификат code signing проверен на тестовой сборке.

Критерии завершения:

- на каждой платформе пользователь может подключаться и отключаться из приложения;
- на каждой платформе приложение определяет active tunnel interface;
- protected routes применяются при подключении на обеих платформах;
- расхождения между платформами сведены к таблице 19.4 ТЗ.

## Phase 4: Watchdog и Fail-Closed

Цель: защитить маршрутизацию при реальных сбоях.

Результаты:

- событийный watchdog с контрольной сверкой;
- route repair;
- автопереключение транспортов по политике и память последнего удачного (`FR-260`, `FR-261`);
- health probes: проверка, что трафик реально идет через туннель (`FR-056`);
- DNS через резолвер tunnel-точки на время подключения, переключатель в режиме эксперта (`FR-167`, `FR-167a`);
- предохранитель helper и его проверка M21 (`FR-197`);
- fail-closed policy полностью: pf на macOS, WFP или firewall на Windows;
- явное управление метриками маршрутов на Windows;
- автоматическая отправка важных ошибок подключения через backend;
- ручная кнопка отправки ошибки админам;
- дедупликация и rate-limit error reports;
- режим эксперта: диагностика, «почему X (не) идет через туннель» и живая проба (`FR-168`, `FR-169`), без локальных правил;
- recovery after sleep/wake;
- recovery after network changes;
- redacted логи.

Критерии завершения:

- route drift чинится автоматически на обеих платформах;
- protected destinations не утекают при падении tunnel: leak-тесты L1-L10 пройдены и на macOS, и на Windows;
- на Windows приложение корректно уживается с включенным Windows Firewall и сторонним VPN-клиентом;
- важные ошибки подключения попадают админам без чувствительных данных;
- пользователь может одной кнопкой отправить ошибку, если она не отправилась автоматически;
- приложение восстанавливается после sleep/wake без ручной чистки маршрутов.

## Phase 5: Packaging, updater и distribution

Цель: подготовить устанавливаемые и обновляемые build для macOS и Windows из одного релизного пайплайна.

Общие результаты:

- сборка обеих платформ из CI по одному тегу с одинаковой версией;
- app icon, versioning;
- встроенный updater Tauri v2 по модели `FR-084`: подписанные артефакты (`app.tar.gz` + `.sig` для macOS, установщик + `.sig` для Windows), манифест `latest.json` на VPN-хосте, обновление helper с проверкой совместимости протокола; обязателен для 1.0 (`FR-082`);
- install/uninstall cleanup checks.

macOS:

- Apple Silicon build artifact;
- DMG packaging;
- signing и notarization.

Windows:

- x64 build artifact;
- установщик MSI или NSIS;
- подпись установщика и исполняемых файлов, проверка прохождения SmartScreen;
- корректная установка и удаление службы helper.

Критерии завершения:

- tester может установить приложение из DMG на macOS и из установщика на Windows;
- приложение запускается после перезагрузки на обеих платформах, если включен autostart;
- uninstall удаляет helper/службу, routes и config без сломанной сети на обеих платформах;
- обновление с предыдущей версии через updater проходит на обеих платформах с сохранением активации; проверены подмена артефакта (отклонена по подписи), недоступность одного артефакта (обновление не начинается), отказ обновления helper (приложение работает на прежнем helper с предупреждением) и прерванная установка (старая версия остается запускаемой);
- релиз содержит оба артефакта; выпуск с одним артефактом не публикуется.

## Phase 6: Backend API и админка

Цель: сделать control plane безопасным для реальных пользователей и видимым для администратора.

Результаты:

- production-доведение client auth endpoints из этапа Phase 2 → 3, регистрация peer по публичному ключу устройства; общая реализация и контрактные тесты переиспользуются;
- IKEv2 на зарубежном сервере: strongSwan, публично доверенный сертификат, EAP-учетки per device из `client-api` (`API-115`);
- настройки политики и `trialDays` в `client-api`, admin API для админ-панели, аудит (`API-130`-`API-136`);
- бот читает `trialDays` из `client-api`, обрабатывает `intent=trial` и передает роль `admin` при подтверждении входа;
- route list versioning и content hash;
- pull списка из дашборда, агрегация и публикация с версией и хешем (`API-110`, `FR-165`);
- subscription enforcement;
- app version policy;
- endpoint для error reports, Telegram-уведомления, дедупликация и rate-limit;
- endpoint диагностики;
- команды бота `/devices`, `/report`, `/clientstatus` как обертки над admin API (`API-005`-`API-007`), веб-админка дашборда не меняется;
- `client-api` на зарубежном сервере: локальный провизионинг peer и сверка с БД (`API-009`, `API-095`-`API-098`), pull списка и статусов из дашборда, пересылка ошибок в `/api/notify`, admin API для бота и админки (`API-110`-`API-124`, API_SPEC 11);
- сервисные токены со scope для endpoints пользователей дашборда (`API-122`);
- бот: обработка `/start link_<код>` для подтверждения входа из приложения и кнопка «Активировать приложение» в меню и карточке пользователя; таблицы устройств и кодов - в БД `client-api`.

Критерии завершения:

- expired users не получают usable route updates;
- revoked devices перестают работать после token revocation;
- админы получают только важные ошибки подключения, а не поток шумных событий;
- изменения route list доходят до клиентов в рамках sync interval;
- администратор видит список устройств с версиями и возрастом whitelist через `/devices` и карточки отчетов через `/report` в боте; раздел отчетов в приложении - 1.1 (`FR-284`).

## Релиз 1.0: macOS и Windows

Отдельной фазы под Windows нет: Windows-адаптер, установщик и CI-раннер входят в фазы 1-5 наравне с macOS.

Условия выпуска 1.0:

- Definition of Done раздела 2.4 ТЗ выполнен для обеих платформ;
- ручная матрица M1-M24 плюс W1-W6 и D1-D2 пройдена на чистых машинах, M3-M9 на каждом транспорте;
- leak-тесты L1-L10 пройдены на обеих платформах и обоих транспортах;
- оба артефакта подписаны и собраны из одного тега;
- updater работает на обеих платформах;
- вход через бота работает обоими способами: deep link с подтверждением и ручной код;
- leak-тесты L1-L11 и тесты helper `T-002` пройдены; интеграционные тесты раздела 21.2 ТЗ зеленые на обоих раннерах.

## Релиз 1.1

Состав по разделу 3.4 ТЗ:

- локальные правила пользователя в режиме эксперта;
- экспорт diagnostic bundle;
- подпись route list Ed25519 и ее проверка на клиенте;
- endpoint событий;
- админ-панель: список маршрутов, отчеты об ошибках, состояние (`FR-283`, `FR-284`);
- несколько tunnel-точек с приоритетом;
- английская локаль;
- Windows on ARM;
- расширения админ-панели: аналитика трафика, grey list.

## Phase 7: Android Technical Spike

Может идти параллельно фазам 5 и 6: не пересекается с desktop-кодом. С WireGuard путь на Android известен - системный tunnel service плюс штатная библиотека, поэтому spike скорее подтверждает, чем открывает.

Результаты:

- Android Tauri shell evaluation;
- system tunnel service prototype;
- background sync prototype;
- always-on behavior analysis;
- Google Play policy checklist;
- decision record.

Критерии завершения:

- принято понятное go/no-go решение по Android implementation path;
- известные риски и required native work задокументированы.

## Phase 8: Android MVP

Цель: выпустить первый Android test build, если spike успешен.

Результаты:

- Android app build;
- tunnel service;
- route policy sync;
- local cache;
- connection status;
- fail-closed equivalent там, где поддерживается;
- internal testing build.

Критерии завершения:

- Android user может установить приложение, подключиться и получать route updates;
- поведение приложения приемлемое при network changes и reboot.
