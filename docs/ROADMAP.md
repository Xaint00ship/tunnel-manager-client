...
- D4: зарубежная tunnel-точка под управлением владельца с WireGuard и агентом провизионинга peer (ТЗ 15.5, API_SPEC 11) - развернуть до фазы 1;
...
- стек утвержден;
- D2 и D4 подтверждены или пересмотрены с обновлением ТЗ;
- агент провизионинга работает на тестовой точке: backend может добавить и удалить peer;
...
- агент провизионинга peer на tunnel-точках и сверка peer с БД (`API-009`, `API-095`-`API-098`, API_SPEC 11);
- кнопка «Активировать приложение» в меню бота и в карточке пользователя, таблицы `client_devices` и `activation_codes` в схеме дашборда.
...
- endpoint событий;
- DNS через резолвер на tunnel-точке (`FR-167`, OQ-8);
