...
Внутренний контракт RU-контур -> `client-api`. Клиентское приложение этот API не видит. Требования - ТЗ, раздел 15.6.

Базовый адрес: `https://<VPN_SERVER_HOST>/admin/v1`. Аутентификация: `Authorization: Bearer <ADMIN_API_TOKEN>`; токен лежит в `.env` дашборда и бота и отличается от `tunnel_api_key`. Доступ ограничен адресом RU-сервера.
