...
- Реализуется в `tunnel-dashboard-backend` (FastAPI) рядом с существующими `/api/*`. Схема Postgres принадлежит этому репозиторию: новые таблицы `client_devices` и `activation_codes` добавляются там, бот их не создает и хранит состояние устройств не в файлах, как сейчас WireGuard-конфиги, а в БД.
...
- `API-056` (MUST). `dnsResolver` - адрес резолвера на tunnel-точке, тот же dnsmasq на tunnel-адресе, что сегодня на сервере. Клиент 1.0 игнорирует поле, клиент 1.1 использует по `FR-167`.
- `API-057` (MUST). Peer для устройства регистрируется на точке через агент провизионинга (раздел 11) в момент активации; ответ `GET /client/v1/endpoint` **MUST** отдаваться только после подтверждения агента, иначе клиент получит адрес, к которому не сможет подключиться.
...
## 11. Агент провизионинга на tunnel-точке

Внутренний контракт backend -> агент. Клиентское приложение этот API не видит. Требования - ТЗ, раздел 15.5.

Базовый адрес: `https://<ENDPOINT_AGENT_HOST>:<PORT>/agent/v1` или через SSH-туннель на loopback. Аутентификация: `Authorization: Bearer <AGENT_TOKEN>`, токен на каждую точку свой, в `.env` backend.

| Метод и путь | Назначение |
| --- | --- |
| `GET /agent/v1/health` | версия агента, интерфейс, публичный ключ точки, подсеть, число peer |
| `GET /agent/v1/peers` | список peer с маркером агента |
| `PUT /agent/v1/peers/{publicKey}` | добавить или обновить peer |
| `DELETE /agent/v1/peers/{publicKey}` | удалить peer |

`PUT /agent/v1/peers/{publicKey}`, запрос:

```json
{
  "deviceId": "dev_9f3a7c2e5b114d8e",
  "addresses": ["<TUNNEL_IPV4>/32"],
  "label": "usr_2c81/dev_9f3a"
}
```
...
- `API-100` (MUST). Агент выполняет только четыре операции выше. Произвольные команды, пути и имена интерфейсов от вызывающего не принимаются; интерфейс задается в конфиге агента.
- `API-101` (MUST). Добавление peer: `wg set <iface> peer <key> allowed-ips <addr>/32` плюс дозапись блока с маркером в wg-quick конфиг, чтобы peer пережил перезапуск интерфейса. Удаление: `wg set ... remove` плюс вырезание блока по маркеру, чужие блоки не трогаются. Это логика `bot/wireguard.py`, перенесенная как есть.
- `API-102` (MUST). Адрес выделяет backend из пула точки, а не агент: backend хранит соответствие устройство -> адрес в БД и обязан не выдать один адрес двум устройствам. Агент отвергает адрес вне подсети интерфейса.
- `API-103` (MUST). `PUT` с тем же ключом и адресом идемпотентен; `PUT` с тем же ключом и другим адресом заменяет `allowed-ips`. `DELETE` отсутствующего ключа возвращает `200` с `"removed": false`.
- `API-104` (MUST). Backend раз в 5 минут сверяет `GET /agent/v1/peers` со списком активных устройств: лишние peer на точке удаляются, недостающие добавляются. Сверка - основной механизм отзыва, вызов при отзыве - ускорение.
- `API-105` (MUST). Агент **MUST** отдавать `dnsResolver` для точки в `health`, чтобы backend не хранил адрес резолвера вручную.
- `API-106` (SHOULD). Агент **SHOULD** отдавать статистику handshake и байтов по peer в `GET /agent/v1/peers`, чтобы админка показывала "онлайн" так же, как сегодня по `wg show` на сервере.
